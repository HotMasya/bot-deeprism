const  Discord = require("discord.js"),
    ...
    module.exports.run = async (bot, message, args, channels, data) => {
        ...
        if(!data.dispatcher) return tools.sendEmbedMessage(bot, message.channel, "error",
        "**В данный момент я ничего не проигрываю!**", "Введите /play чтобы добавить треки в очередь.");

        if(data.dispatcher.paused) return tools.sendEmbedMessage(bot, message.channel, "error",
